...
Integrates with bubble indicator and multi-stock analysis.
"""
...
class GoogleTrendsSentiment:
    ...
    def get_sentiment_timeseries(self,
        ...
        """
        Get sentiment scores over time as a DataFrame.

        Useful for integrating with bubble indicator and backtesting.
        ...
        Returns:
            DataFrame with date index and sentiment columns
        """
        if keywords is None:
        ...
        trends_df = self.download_trends(keywords, start_date, end_date)

        if trends_df.empty:
            return pd.DataFrame()

        # Calculate rolling sentiment
        result = pd.DataFrame(index=trends_df.index)

        for keyword in trends_df.columns:
            result[f'{keyword}_interest'] = trends_df[keyword]

            # Invert bearish keywords
            if any(bear in keyword.lower() for bear in ['crash', 'bubble', 'correction', 'recession', 'sell']):
                result[f'{keyword}_sentiment'] = 100 - trends_df[keyword]
            else:
                result[f'{keyword}_sentiment'] = trends_df[keyword]

        # Overall sentiment (average of all sentiment columns)
        sentiment_cols = [c for c in result.columns if c.endswith('_sentiment')]
        result['overall_sentiment'] = result[sentiment_cols].mean(axis=1)
...
def download_ai_nvda_sentiment(start_date: str = None,
    ...
    This is the main deliverable function for sentiment analysis.
...
def get_bubble_sentiment_indicator(ticker: str = 'NVDA',
                                    start_date: str = None,
                                    end_date: str = None) -> Dict:
    """
    Get sentiment indicator specifically for bubble analysis.

    Designed for integration with the bubble indicator feature.
    ...
    Returns:
        Dictionary with bubble-related sentiment metrics
    """
    if end_date is None:
        end_date = datetime.now().strftime('%Y-%m-%d')
    if start_date is None:
    ...
    # Keywords specific to bubble analysis
    keywords = [
        f'{ticker} stock',
        f'{ticker} bubble',
        'AI bubble',
    ...
    # Calculate bubble risk score (0-100, higher = more bubble-like)
    keyword_scores = result.get('keyword_scores', {})

    bubble_keywords_interest = []
    for keyword, scores in keyword_scores.items():
        if 'bubble' in keyword.lower() or 'crash' in keyword.lower():
            bubble_keywords_interest.append(scores['current'])

    bubble_risk = np.mean(bubble_keywords_interest) if bubble_keywords_interest else 50

    return {
        'ticker': ticker,
        'sentiment_score': result['sentiment_score'],
        'bubble_risk_score': float(bubble_risk),
        'interpretation': result['interpretation'],
        'bubble_risk_level': _interpret_bubble_risk(bubble_risk),
        'keyword_breakdown': keyword_scores,
        'date_range': result['date_range']
    }


def _interpret_bubble_risk(score: float) -> str:
    """Convert bubble risk score to interpretation."""
    if score >= 70:
        return "High Risk - Elevated bubble concern searches"
    elif score >= 50:
        return "Moderate Risk - Some bubble concern"
    elif score >= 30:
        return "Low Risk - Minimal bubble searches"
    else:
        return "Very Low Risk - Little bubble concern"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("Bubble Sentiment Indicator (NVDA)")
        print("=" * 60)

        bubble_result = get_bubble_sentiment_indicator('NVDA')
        print(f"\nTicker: {bubble_result['ticker']}")
        print(f"Sentiment Score: {bubble_result['sentiment_score']:.1f}/100")
        print(f"Bubble Risk Score: {bubble_result['bubble_risk_score']:.1f}/100")
        print(f"Bubble Risk Level: {bubble_result['bubble_risk_level']}")

        print("\n" + "=" * 60)
        print("Sentiment Timeseries (for backtesting)")
        print("=" * 60)

        ts_result = sentiment.get_sentiment_timeseries()
        if not ts_result.empty:
            print(f"\nTimeseries shape: {ts_result.shape}")
            print(f"Date range: {ts_result.index[0]} to {ts_result.index[-1]}")
            print(f"\nLatest values:")
            print(ts_result.tail(5))
    ...
    print("  - download_ai_nvda_sentiment() function")
    print("  - get_sentiment_score() function")
    print("  - get_fear_greed_proxy() method")
    print("  - get_bubble_sentiment_indicator() for bubble feature integration")
    print("  - get_sentiment_timeseries() for backtesting")
